...
class Tools extends Pages {
	...
	public function render_tool_card( $tool ) {
		...
		?>
		<div class="tainacan-tool" data-tool-id="<?php echo esc_attr( $id ); ?>"<?php echo $destructive ? ' data-destructive="1"' : ''; ?> data-required-params="<?php echo esc_attr( wp_json_encode( $this->get_required_param_names( $tool->get_params() ) ) ); ?>">
			<div class="tainacan-tool-main">
				<h2 class="tainacan-tool-title"><?php echo esc_html( $tool->get_name() ); ?></h2>
				<?php if ( $tool->get_description() ) : ?>
					<p class="tainacan-tool-description"><?php echo esc_html( $tool->get_description() ); ?></p>
				<?php endif; ?>
				<div class="tainacan-tool-form">
					<?php echo $this->render_tool_form_fields( $tool->get_params(), $id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</div>
				<div class="tainacan-tool-run">
					<button type="button" class="button button-primary tainacan-tool-run-btn"><?php esc_html_e( 'Run', 'tainacan' ); ?></button>
				</div>
			</div>
			<div class="tainacan-tool-output" aria-live="polite"><div class="tainacan-tool-output-placeholder"><?php esc_html_e( 'Output messages will appear here.', 'tainacan' ); ?></div></div>
		</div>
		<?php
	}
